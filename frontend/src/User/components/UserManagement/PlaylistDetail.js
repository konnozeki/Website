...
const PlaylistDetail = () => {
  ...
  return (
    <div style={{ textAlign: 'center', backgroundColor: "rgb(28,28,28)", height: "auto", minHeight: "100vh" }}>
      ...
      <div>
        {playlistData.episodes.length !== 0 ? <Title level={2} style={{color: 'white'}}>Danh sách</Title> : <></>}
                ...
                >
                  <div onClick={() => handleEpisodeClick(episode)} style={{ cursor: 'pointer' }}>
                    ...
                    <Space direction="vertical" style={{ marginLeft: '10em' }}>
                      <Text style={{ fontSize: 16, fontWeight: 'bold', color: 'white' }}>Tập phim {episode.index}</Text>
                      <Text style={{color: 'white'}}>{episode.film_episode.description}</Text>
                    </Space>
                  </div>
                  <Button
                    type="primary"
