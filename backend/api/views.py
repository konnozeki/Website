...
class AverageRateFilmView(generics.RetrieveAPIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        average_rate = RateFilm.objects.filter(film=film).aggregate(Avg('rate'))['rate__avg']
...
class AverageRateFilmEpisodeView(generics.RetrieveAPIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        average_rate = RateFilmEpisode.objects.filter(film_episode=film_episode).aggregate(Avg('rate'))['rate__avg']
...
class CreateRateFilmEpisodeView(generics.CreateAPIView):
    ...
    def perform_create(self, serializer):
        ...
        existing_rate = RateFilmEpisode.objects.filter(user=user, film_episode=film_episode).first()
        if existing_rate:
            serializer.instance = existing_rate
        else:
...
class UpdateDeleteRateFilmEpisodeView(generics.RetrieveUpdateDestroyAPIView):
    ...
    def get_object(self):
        film_episode_slug = self.kwargs['film_episode_slug']
        user = self.request.user
        return get_object_or_404(RateFilmEpisode, user=user, film_episode__slug=film_episode_slug)
...
class ListCommentsForFilmView(generics.ListAPIView):
    ...
    def get_queryset(self):
        film_slug = self.kwargs['film_slug']
        comment_ids = CommentFilm.objects.filter(film__slug=film_slug).values_list('comment_id', flat=True)
