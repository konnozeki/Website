...
class RetrieveUpdateDeleteUserView(generics.RetrieveUpdateDestroyAPIView):
    ...
    def put(self, request, *args, **kwargs):
        ...
        if user_serializer.is_valid():
            ...
            profile_data = {
                "birth": request.data.get("birth", user_profile.birth),
            ...
            if profile_serializer.is_valid():
                ...
                return JsonResponse(
                    {"message": "Update user profile unsuccessful", "errors": profile_serializer.errors},
                    status=status.HTTP_400_BAD_REQUEST,
                )
        else:
            return JsonResponse(
                {"message": "Update user unsuccessful", "errors": user_serializer.errors},
                status=status.HTTP_400_BAD_REQUEST,
            )
...
class UpdateDeleteFilmEpisodeView(generics.RetrieveUpdateDestroyAPIView):
    ...
    def update(self, request, *args, **kwargs):
        data = request.data.copy()  # Tạo một bản sao có thể thay đổi của QueryDict
        ...
        serializer = self.get_serializer(film_episode, data=data)
        if serializer.is_valid():
            serializer.save()
            return JsonResponse(
